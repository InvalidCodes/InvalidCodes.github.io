...
"""), self.root)
    ...
    def test_month_precision_and_reading_notes_archive(self):
        ...
        self.assertNotIn("Book reflection", main)
        self.assertNotIn("Another book", main)
        homepage = (output.parent / "index.html").read_text()
        self.assertIn("Ordinary post", homepage)
        self.assertNotIn("Book reflection", homepage)
        self.assertNotIn("Another book", homepage)
        ordinary = (output / "ordinary.html").read_text()
        self.assertNotIn("Book reflection", ordinary)
        self.assertNotIn("Another book", ordinary)
    ...
    def test_build_order_links_assets_and_markdown(self):
        ...
        (self.root / "assets/.DS_Store").write_text("OS metadata")
        (self.root / "assets/README.md").write_text("Asset documentation")
        (self.root / "source-assets/figures").mkdir(parents=True)
        (self.root / "source-assets/figures/source.pdf").write_bytes(b"source figure")
        ...
        self.assertFalse((output / "assets/README.md").exists())
        self.assertFalse((output / "source-assets").exists())
